...
- On changes that affect the UI, the UI threading or how the app renders things, you must always run the full cold start performance harness to make sure your new changes don't cause a performance regression.
Bug fixes, exception handling and other minor things do not require this. don't run the performance harnesses unless it's actually needed.
