...
class BackgroundWorker(
    ...
    override fun startWork(): ListenableFuture<Result> {
        ...
        engine?.let { registerDualMateWidgetPlugin(it) }
    ...
    private fun registerDualMateWidgetPlugin(backgroundEngine: FlutterEngine) {
        try {
            val pluginClass = Class.forName("com.fariszr.dualmate.flutter.AndroidScheduleTodayWidget")
            val plugin = pluginClass.getDeclaredConstructor().newInstance() as FlutterPlugin
            backgroundEngine.plugins.add(plugin)
        } catch (_: ClassNotFoundException) {
            // DualMate custom widget bridge is optional for upstream users.
        } catch (_: IllegalStateException) {
            // Plugin already registered for this engine.
        } catch (_: Exception) {
            // Best-effort registration; task execution must continue.
        }
    }
