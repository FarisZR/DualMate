...
class DhbwAppCanteenSource {
  ...
  Future<List<DailyMenu>> loadWeek(
    ...
    final sitePayload = await _loadSitePayload(site, cancellationToken);
    cancellationToken?.throwIfCancelled();

    final menus = await compute(
      _parseDhbwAppWeek,
      _DhbwAppWeekParseRequest(
        sitePayload: sitePayload,
        mensaId: mensaId,
        weekStart: weekStart,
      ),
      debugLabel: 'dhbwAppCanteenParseWeek',
    );
    cancellationToken?.throwIfCancelled();
  ...
  Future<List<dynamic>> _loadSitePayload(
    ...
    if (cached != null &&
        ...
        (cancellationToken == null || cached.isComplete)) {
      return cached.payload;
    }

    if (cancellationToken != null) {
      final payload = await _fetchAndDecodeSitePayload(site, cancellationToken);
      _sitePayloadCache[site] = _DhbwAppSitePayloadCacheEntry(
        createdAt: _now(),
        payload: Future<List<dynamic>>.value(payload),
        isComplete: true,
      );
      return payload;
    }
    ...
    try {
      final result = await payload;
      if (_sitePayloadCache[site]?.payload == payload) {
        _sitePayloadCache[site] = _DhbwAppSitePayloadCacheEntry(
          createdAt: now,
          payload: Future<List<dynamic>>.value(result),
          isComplete: true,
        );
      }
      return result;
...
List<dynamic> _decodeDhbwAppSitePayload(String body) {
...
List<DailyMenu> _parseDhbwAppWeek(_DhbwAppWeekParseRequest request) {
  final mensaEntry = request.sitePayload
      .whereType<Map<String, dynamic>>()
      .firstWhere(
        (entry) => _mensaInfoId(entry) == request.mensaId,
        orElse: () => const <String, dynamic>{},
      );
  if (mensaEntry.isEmpty) {
    return _emptyWeek(request.weekStart);
  }

  return _menusFromEntry(mensaEntry, request.weekStart);
}
...
class _DhbwAppWeekParseRequest {
  final List<dynamic> sitePayload;
  final int mensaId;
  final DateTime weekStart;

  const _DhbwAppWeekParseRequest({
